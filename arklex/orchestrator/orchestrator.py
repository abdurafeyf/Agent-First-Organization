        ...
        while not FINISH:
            ...
            if node_info["id"] not in self.env.workers and node_info["id"] not in self.env.tools:
                ...
                if node_info["id"] == self.env.name2id["DefaultWorker"]:
                    ...
                    FINISH = True
                    break
                elif node_info["id"] == self.env.name2id["MarketInfoMessageWorker"]:
                    logger.info("Skip ReAct framework because it is hard to distinguish between the MarketInfoMessageWorker and the RESPOND action")
